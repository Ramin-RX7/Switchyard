...
**IP access control** ([docs/routing.md](docs/routing.md#ip-access-control)): per-location `whitelist`/`blacklist` (single IP or CIDR, v4/v6). Evaluated in the `Decider` right after the location matches (before method routing/backend selection), so it applies to proxy/static/response locations; blacklist wins over whitelist, non-empty whitelist is allow-list only, empty/omitted = unrestricted. Denial → **403** via the configurable `forbidden` responder (SDK `p.Forbidden`). Uses `RemoteAddr` (custom `AccessController` via `loc.Access` for `X-Forwarded-For`/other logic). Bad entries fail fast at startup.
